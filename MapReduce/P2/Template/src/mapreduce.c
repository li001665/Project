...
int main(int argc, char *argv[]) {
    ...
    if((msgctl(msgid, IPC_RMID, 0)) == -1) {			// check whether the message queue is removed successfully
		printf("ERROR: message queue of chunk data removed failed\n");
		return -1; 					// terminate
	}
    ...
    if((msgctl(msgid, IPC_RMID, 0)) == -1) {// check whether the message queue is removed successfully
		printf("ERROR: message queue of shuffle removed failed\n");
		return -1; 
	}
	return 0;
}
